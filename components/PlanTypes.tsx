    ...
    <section id="planos" className="py-20 sm:py-32 bg-background relative" aria-labelledby="plans-heading">
      <div className="max-w-7xl mx-auto px-6 sm:px-4">
        <div className="text-center mb-12 sm:mb-20">
          <h2 id="plans-heading" className="text-2xl sm:text-5xl font-extrabold text-accent mb-4 sm:mb-6 tracking-tight">{planTypes.title}</h2>
        ...
        <div className="grid lg:grid-cols-3 gap-6 sm:gap-10" role="list">
          {planTypes.items.map((plan) => (
            <article key={plan.id} className="bg-surface rounded-[2rem] overflow-hidden shadow-sm flex flex-col group border border-accent/5 transition-all active:scale-[0.98]" role="listitem" aria-labelledby={`plan-${plan.id}`}>
              ...
              <div className="p-6 sm:p-10 flex flex-col flex-grow">
                <h3 id={`plan-${plan.id}`} className="text-xl sm:text-2xl font-black text-accent mb-3 sm:mb-4 tracking-tight">{plan.title}</h3>
                <p className="text-text-muted mb-6 sm:mb-8 flex-grow leading-relaxed text-xs sm:text-base">{plan.description}</p>
                <a 
                  href="#contato" 

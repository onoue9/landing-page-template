    ...
    <section id="faq" className="py-20 sm:py-32 bg-background" aria-labelledby="faq-heading">
      <div className="max-w-3xl mx-auto px-6 sm:px-4">
        <AnimatedSection className="text-center mb-12 sm:mb-16">
          <h2 id="faq-heading" className="text-2xl sm:text-5xl font-extrabold text-accent tracking-tight mb-4">
            {faq.title}
          </h2>
        ...
        <div className="space-y-4">
          {faq.items.map((item, index) => (
            <AnimatedSection key={index} delay={index * 75}>
              <div className="bg-surface rounded-2xl sm:rounded-3xl overflow-hidden border border-accent/5 shadow-sm hover:border-primary/20 transition-colors duration-300">
                <button
                ...
                >
                  ...
                  <ChevronDown
                    className={`w-5 h-5 sm:w-6 sm:h-6 text-text-muted transition-transform duration-300 flex-shrink-0 ${openIndex === index ? 'rotate-180 text-primary' : ''}`}
                    aria-hidden="true"
                  />
                </button>
                <div

...
const { site, content, theme } = demoConfigs.odontologia;
...
export default function OdontologiaPreviewClient() {
  ...
  const ctaMessage = encodeURIComponent(
    `Olá! Testei o preview da landing page de odontologia e gostei.\n\nDados da minha clínica:\n- Nome: ${fields.namePrefix}${fields.nameHighlight}\n- WhatsApp: ${fields.whatsapp}\n- E-mail: ${fields.email}\n\nPodemos conversar?`
  );
    ...
    <div className="flex h-screen overflow-hidden bg-slate-100" style={{ fontFamily: 'sans-serif' }}>
      ...
      <aside className={[
        ...
        <nav className="flex items-center gap-1 px-4 py-2 border-b border-slate-100">
          ...
          <a href="/demo/odontologia" className="text-[11px] font-semibold text-slate-400 hover:text-slate-600 transition-colors whitespace-nowrap">
            Odontologia
          </a>
          <ChevronRight className="w-3 h-3 text-slate-300 shrink-0" />
          <span className="text-[11px] font-semibold text-slate-600 whitespace-nowrap">Preview</span>
        </nav>
        <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
          <div>
            <h1 className="text-sm font-black text-slate-800">Configurador</h1>
            <p className="text-[10px] text-slate-400">Odontologia · Edite ao vivo</p>
          </div>
          <button
            onClick={resetAll}
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-6">
          <FormSection title="Clínica">
            <div className="grid grid-cols-2 gap-2">
              <Field label="Prefixo" value={fields.namePrefix} onChange={set('namePrefix')} placeholder="Odonto" />
              <Field label="Destaque" value={fields.nameHighlight} onChange={set('nameHighlight')} placeholder="Vita" />
            </div>
            <Field label="Slogan" value={fields.tagline} onChange={set('tagline')} />
            <Field label="WhatsApp (DDI+número)" value={fields.whatsapp} onChange={set('whatsapp')} placeholder="5531999999999" />
            ...
            <div className="grid grid-cols-4 gap-1.5">
              {(Object.keys(PRESETS) as PresetKey[]).map((key) => (
                <button
                  ...
                  className="py-1.5 text-[10px] font-bold rounded-lg border border-slate-200 hover:border-slate-400 transition-colors capitalize"
        ...
        <div className="flex-1 overflow-auto bg-slate-200 p-4">
          ...
          >
            <iframe
              key={device}
              ref={iframeRef}
              src="/demo/odontologia"
              title="Preview da landing page de odontologia"

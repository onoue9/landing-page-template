...
const { site, content, theme } = demoConfigs.saude;
...
export default function SaudePreviewClient() {
  ...
  // Detect screen size: set initial device + sidebar state
  useEffect(() => {
    const update = () => {
  ...
  // Auto-select device based on screen width on first load
  useEffect(() => {
    const w = window.innerWidth;
    ...
    <div className="flex h-screen overflow-hidden bg-slate-100" style={{ fontFamily: 'sans-serif' }}>
      ...
      <aside className={[
        ...
        <nav className="flex items-center gap-1 px-4 py-2 border-b border-slate-100">
          ...
          <a href="/demo/saude" className="text-[11px] font-semibold text-slate-400 hover:text-slate-600 transition-colors whitespace-nowrap">
            Plano de Saúde
          </a>
          <ChevronRight className="w-3 h-3 text-slate-300 shrink-0" />
        ...
        <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
          <div>
            ...
            <p className="text-[10px] text-slate-400">Plano de Saúde · Edite ao vivo</p>
          </div>
          <button
            onClick={resetAll}
        ...
        <div className="flex-1 overflow-auto bg-slate-200 p-4">
          ...
          >
            <iframe
              ...
              ref={iframeRef}
              src="/demo/saude"
              title="Preview da landing page de saúde"
